...
## Operador pipe %>% 
::: {.incremental .small-par}
- Primero instalaremos la librería dplyr de la siguiente manera:
:::
. . .
```{r}
#| echo: TRUE
...
- En el ejemplo estamos aplicando la función head al dataframe vuelos
:::
...
- Ejemplo: cortar las cinco primeras filas de "vuelos"
:::
. . .
```{r}
#| echo: TRUE
...
::: {.incremental .small-par}
- **Funciones slice**
- Ejemplo: cortar las cinco últimas filas de "vuelos"
:::
. . .
```{r}
#| echo: TRUE
...
::: {.incremental .small-par}
- **Funciones slice**
- - A diferencia de la función filter, slice selecciona las filas por su **posición** numérica en el marco de datos.
- Ejemplo: obtener filas especificadas
:::
. . .
```{r}
#| echo: TRUE
...
## Anidación de funciones
::: {.incremental .small-par}
- Utilizar la función count() para contar la frecuencia de ocurrencia de combinaciones únicas de los valores "mes" y "dia"
- Luego, utilizar la función filter() para seleccionar solo aquellas filas donde el valor de la columna "dia" sea igual a 1.
- Finalmente, utilizar la función slice_head() para mostrar las primeras filas del resultado obtenido, que contiene la frecuencia de vuelos para cada mes en el primer día del mes.
:::
. . .
```{r}
#| echo: TRUE
...
#| echo: TRUE
#| eval: FALSE
vuelos %>% count(mes, dia) %>% filter(dia == 1) %>% slice_head(n = 5)
```
...
#| echo: TRUE
#| eval: TRUE
flores <- read_excel("clase_2/flores.xlsx")
names(flores)
```
